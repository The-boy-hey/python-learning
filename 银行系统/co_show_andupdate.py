...
class CorporateModifyApp(QWidget):
    ...
    def initUI(self):
        ...
        self.btn_update = QPushButton('修改信息')
        self.btn_add_corporate = QPushButton('前往添加企业信息')
        self.btn_detail_mg_co = QPushButton('返回查看企业列表')
        ...
        vbox.addWidget(self.btn_add_corporate)
        vbox.addWidget(self.btn_detail_mg_co)
        self.setLayout(vbox)

        # 连接信号和槽
        self.btn_load.clicked.connect(self.load_data)
        self.btn_update.clicked.connect(self.update_data)
        self.btn_add_corporate.clicked.connect(self.gotoaddco)
        self.btn_detail_mg_co.clicked.connect(self.detail_mg_co)
    ...
    def update_data(self):
        ...
        legalman = self.le_legalman.text().strip()

        if co_id.isdigit():
            sql = "UPDATE t_corporate SET co_name = %s, major_work = %s, regist_address = %s, regist_date = %s, regist_money = %s, co_number = %s ,legalman = %s WHERE co_id = %s"
            try:
                self.cursor.execute(sql, (co_name, major_work, regist_address, regist_date, regist_money, co_number,legalman, co_id))
                self.db.commit()
    ...
    def gotoaddco(self):
        self.hide()  # 主界面的隐藏
        self.gtac = add_corporate.Ui_MainWindow()
        #注册界面打开
        self.gtac.show()
